...
Peningkatan akurasi (vs versi awal):
  ...
  5. Continuous Collision Detection (sapu lintasan penuh) → seluruh jejak
     pergelangan dari onset pukulan hingga apex diuji segmen-per-segmen ke
     target, sehingga tunneling (tinju terlalu cepat) tak menyebabkan miss
     di titik mana pun pada lintasan, bukan hanya dekat apex.
"""
...
# --------------------------------------------------------------------------
PUNCH_SPEED_MIN = 1.0      # kecepatan pergelangan minimal (unit-layar/detik)
...
TARGET_RADIUS = 0.20       # radius zona target (koordinat ternormalisasi)
FRAME_ASPECT = 9.0 / 16.0  # tinggi/lebar frame kamera 16:9 — dipakai koreksi hit detection
...
APEX_WINDOW = 0.13         # detik; beri waktu cukup agar pergelangan mencapai target
...
# Continuous Collision Detection — sapu LINTASAN PENUH pukulan.
# Jejak pergelangan dari onset (beberapa frame sebelum pemicu) hingga apex
# disimpan, lalu SETIAP ruas garisnya diuji ke target → cegah tunneling di
# seluruh lintasan, bukan hanya dekat apex.
TRAIL_HISTORY = 6  # frame ramp-up yang ikut di-seed ke jejak saat pukulan dipicu
...
ELBOW_STRAIGHT = 145  # derajat; guard ≈ 80–100°, julur penuh ≈ 160–175°

# Kecepatan z (kedalaman) — tinju lurus ke depan mengubah z secara signifikan.
# Z_WEIGHT: bobot komponen z dalam speed_mag (z MediaPipe lebih noise dari x,y).
Z_WEIGHT = 0.6

# One-Euro filter — parameter untuk koordinat ternormalisasi.
# Disetel SANGAT RESPONSIF: filter harus transparan untuk gerakan cepat (jab <100ms).
ONE_EURO_MIN_CUTOFF = 2.5
ONE_EURO_BETA = 2.0
ONE_EURO_DCUTOFF = 10.0
...
VISIBILITY_THRESHOLD = 0.3  # diturunkan: tinju ke depan bisa turunkan skor visibility
...
# Titik 3D ter-filter hasil One-Euro (x, y dari frame ternormalisasi; z = kedalaman).
...
class _WristFilter:
    """One-Euro untuk titik 3D pergelangan (x, y, z terpisah).
    z difilter lebih halus karena lebih noise dari x,y."""
    ...
    def __init__(self):
        ...
        self.fz = _OneEuro(ONE_EURO_MIN_CUTOFF, ONE_EURO_BETA * 0.5, ONE_EURO_DCUTOFF)
    ...
    def __call__(self, wrist, dt):
        z = getattr(wrist, "z", 0.0) or 0.0
        return _Pt(self.fx(wrist.x, dt), self.fy(wrist.y, dt), self.fz(z, dt))
...
class PunchAnalyzer:
    ...
    def _init_hand_state(self):
        ...
        self._wrist_hist = {"left": deque(maxlen=TRAIL_HISTORY),  # jejak ramp-up
    ...
    def _elbow_angle(self, landmarks, hand):
        """Sudut siku (derajat) dari vektor bahu→siku dan siku→pergelangan (2D).
        Guard ≈ 80–100°; julur penuh ≈ 160–175°. Konsisten untuk SEMUA arah tinju
        termasuk lurus ke depan (arah kamera) yang tak terdeteksi oleh ext 2D."""
        sh_idx = HANDS[hand][0]
        el_idx = ELBOWS[hand]
        wr_idx = HANDS[hand][1]
        if not self._visible(landmarks, (sh_idx, el_idx, wr_idx)):
            return 0.0
        sh = landmarks[sh_idx]
        el = landmarks[el_idx]
        wr = landmarks[wr_idx]
        v1x, v1y = sh.x - el.x, sh.y - el.y   # vektor siku → bahu
        v2x, v2y = wr.x - el.x, wr.y - el.y   # vektor siku → pergelangan
    ...
    def analyze(self, landmarks, frame_time=None):
        ...
        for hand in HANDS:
            ...
            # Kecepatan 3D (x, y, z) — z ditimbang karena lebih noise.
            prev = self._prev_wrist[hand]
            if prev is not None:
                self._vel_buf[hand].append((
                    (wrist.x - prev.x) / dt,
                    (wrist.y - prev.y) / dt,
                    (wrist.z - prev.z) / dt * Z_WEIGHT,  # z tertimbang
                ))
            self._prev_wrist[hand] = wrist
            self._wrist_hist[hand].append(wrist)   # jejak untuk CCD lintasan penuh
            ...
            if self._vel_buf[hand]:
                ...
                vz = sum(v[2] for v in self._vel_buf[hand]) / len(self._vel_buf[hand])
            else:
                vx = vy = vz = 0.0
            speed_mag = math.sqrt(vx * vx + vy * vy + vz * vz)

            # Komponen kecepatan KELUAR 2D (menjauh dari bahu).
            if ext_dist > 1e-6:
                ox = (wrist.x - shoulder.x) / ext_dist
                oy = (wrist.y - shoulder.y) / ext_dist
            ...
            # Sudut siku — metrik ekstensi arah-independen.
            # Tinju lurus ke depan (arah kamera): ext 2D hampir tidak berubah,
            # tapi siku tetap meluruskan → arm_forward menangkap ini.
            elbow_ang = self._elbow_angle(landmarks, hand)
            arm_forward = elbow_ang >= ELBOW_STRAIGHT

            self._cycle_peak[hand] = max(self._cycle_peak[hand], ext)
            reach = self._reach_est[hand]

            # 1) Pukulan menunggu apex → rekam jejak lintasan, lalu selesaikan.
            pend = self._pending[hand]
            if pend is not None:
                pend["trail"].append(wrist)   # tiap frame menambah ruas lintasan
                new_peak = ext > pend["best_ext"] + APEX_EPS
                if ext > pend["best_ext"]:
                    pend["best_ext"] = ext
                # Selama masih di sekitar puncak, simpan kecepatan terbaik.
                ...
                if at_apex:
                    resolved.append((hand, pend["trail"], pend["speed_ms"]))
            ...
            # 3) Picu pukulan: ter-armed, cepat, dan lengan terjulur ke arah yang benar.
            vz_real = vz / Z_WEIGHT if Z_WEIGHT > 0 else 0.0
            forward_speed = max(0.0, -vz_real)   # positif = pergelangan mendekati kamera
            dir_ok = (outward_speed > self.speed_min * 0.3
                      or forward_speed > self.speed_min * 0.5)

            if (self._armed[hand] and self._pending[hand] is None
                    and (ext >= self.extend_frac * self._reach_est[hand] or arm_forward)
                    and speed_mag >= self.speed_min
                    and dir_ok):
                self._armed[hand] = False
                init_stagnant = 0

                # SEED JEJAK LINTASAN dari history ramp-up (onset → frame pemicu),
                # lalu tiap frame berikutnya menambah ruas sampai apex.
                trail = list(self._wrist_hist[hand]) or [wrist]

                self._pending[hand] = {"start_t": now, "best_ext": ext,
                                       "trail": trail, "speed_ms": speed_ms,
    ...
    def _drop_hand(self, hand):
        ...
        if pend is not None:
            now = time.monotonic()
            self._pending[hand] = None
            self._resolve_punch(hand, pend["trail"], pend["speed_ms"], now=now)
    ...
    def _trail_hits_target(trail, tgt):
        """Continuous Collision Detection: uji SETIAP ruas garis pada jejak
        lintasan pukulan ke lingkaran target. Hit bila ada satu ruas yang
        jarak-terdekat-nya ke pusat target <= radius. Aspek rasio frame
        diterapkan pada sumbu Y agar lingkaran target tak lonjong."""
        cx = tgt["x"]
        cy = tgt["y"] * FRAME_ASPECT
        r = tgt["r"]
        pts = trail or []
        if not pts:
            return False
        if len(pts) == 1:
            p = pts[0]
            return math.hypot(cx - p.x, cy - p.y * FRAME_ASPECT) <= r
        ...
        for a, b in zip(pts, list(pts)[1:]):
            ...
            if ab_sq < 1e-9:
                dist = math.hypot(cx - ax, cy - ay)   # ruas degenerasi → titik
            else:
                # Proyeksikan pusat target ke ruas garis, clamp ke [0,1].
    ...
    def _resolve_punch(self, hand, trail, speed_ms, now):
        """Hitung satu pukulan sah: hit/miss memakai sapuan LINTASAN PENUH
        (Continuous Collision Detection), lalu skor & kombo."""
        self.stats.punches += 1
        self.stats._speeds.append(speed_ms)
        tgt = self.active_target

        hit = tgt is not None and self._trail_hits_target(trail, tgt)
...
if __name__ == "__main__":
    ...
    def make_pose(lw=(0.40, 0.45), rw=(0.60, 0.45),
        ...
        shoulder_width = 0.20. Siku di tengah antara bahu & pergelangan."""
        lm = [Landmark(0.5, 0.5, 0.0, vis) for _ in range(33)]
        lm[LEFT_SHOULDER]  = Landmark(ls[0], ls[1], 0.0, vis)
        lm[RIGHT_SHOULDER] = Landmark(rs[0], rs[1], 0.0, vis)
        lm[LEFT_ELBOW]     = Landmark((ls[0]+lw[0])/2, (ls[1]+lw[1])/2, 0.0, vis)
        lm[RIGHT_ELBOW]    = Landmark((rs[0]+rw[0])/2, (rs[1]+rw[1])/2, 0.0, vis)
        lm[LEFT_WRIST]     = Landmark(lw[0], lw[1], 0.0, vis)
        lm[RIGHT_WRIST]    = Landmark(rw[0], rw[1], 0.0, vis)
        return lm
    ...
    # 11b. CCD lintasan penuh: target di TENGAH lintasan, radius kecil.
    # Apex (y=0.90) jauh dari target (y=0.62) → deteksi titik-apex lama MISS,
    # tapi ruas garis 0.45→0.63 menembus target → CCD lintasan penuh HIT.
    an = PunchAnalyzer(); an.start()
    an.active_target = {"id": "X", "x": 0.40, "y": 0.62, "r": 0.05}
    fb = feed(an, jab_left(0.90))
    check("CCD lintasan penuh -> tembus target di tengah = hit",
          fb["last_event"]["type"] == "hit")
